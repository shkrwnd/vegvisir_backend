...
Revision ID: update_vendors_hours_20251115
...
revision: str = 'update_vendors_hours_20251115'
...
def upgrade() -> None:
    ...
    # Check and alter any VARCHAR columns with length constraints to unlimited VARCHAR
    # This handles cases where the table was created with VARCHAR(32) or similar constraints
    ...
    columns_to_alter = result.fetchall()
    for column_name, max_length in columns_to_alter:
        # Alter column to unlimited VARCHAR
        bind.execute(sa.text(f"ALTER TABLE vendors ALTER COLUMN {column_name} TYPE VARCHAR"))
